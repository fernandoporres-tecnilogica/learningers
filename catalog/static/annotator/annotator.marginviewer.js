...
(function() {
  ...
  Annotator.Plugin.MarginViewer = (function(_super) {
    ...
    MarginViewer.prototype.pluginInit = function() {
      ...
      this.marginData = new Annotator.Plugin.MarginViewerObjectStore([], this.funcObject);
    ...
    MarginViewer.prototype.onAnnotationSelected = function(event) {
      ...
      if (annotations.length > 1) {
        for (i = _i = 0, _ref = annotations.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
          if ($(annotations[i]._marginObject).hasClass("annotation-selected")) {
    ...
    MarginViewer.prototype.convert = function(text) {
      text = Annotator.Util.escape(text || '');
    ...
    MarginViewer.prototype.renderMarginObject = function(annotation) {
      var datetime, delel, perm, user;
      datetime = annotation.created ? this.formattedDateTime(new Date(annotation.created)) : '';
      user = this.annotator.plugins.AnnotateItPermissions && this.annotator.plugins.AnnotateItPermissions.user ? this.annotator.plugins.AnnotateItPermissions.user.userId : 'anonymous';
      perm = annotation.permissions ? annotation.permissions["delete"].indexOf(user) >= 0 : 1;
      delel = annotation.user === user || perm ? '<span class="annotator-marginviewer-delete" style="float: left; direction: ltr;">X</span>' : '';
      return '<div class="annotation"><div class="annotation-text">' + this.convert(annotation.text) + '</div></div>';
    };
    ...
    MarginViewer.prototype.createMarginObject = function(annotation, location, hide) {
      ...
      marginObjects.children(".annotation").children(".annotator-marginviewer-delete").click(function(event) {
    ...
    MarginViewer.prototype.onAnnotationUpdated = function(annotation) {
      return $(annotation._marginObject).children(".annotation-text").html(this.convert(annotation.text));
    };

    MarginViewer.prototype.onMarginMouseIn = function(obj) {
      return $($(obj).closest(".annotation")[0].annotation.highlights).addClass("annotator-hl-uber-temp").removeClass("annotator-hl");
    };

    MarginViewer.prototype.onMarginMouseOut = function(obj) {
      return $($(obj).closest(".annotation")[0].annotation.highlights).addClass("annotator-hl").removeClass("annotator-hl-uber-temp");
    };
    ...
    MarginViewer.prototype.onMarginSelected = function(obj) {
      ...
      marginObject = $(obj).closest(".annotation")[0];
      ...
      if (this.currentSelectedAnnotation !== null) {
        if (annotation.id === this.currentSelectedAnnotation.id) {
          ...
          if (currentMarginObject !== null) {
            ...
            $(currentMarginObject).removeClass("annotation-selected");
          }
        }
      }
      $(marginObject).addClass("annotation-selected");
    ...
    MarginViewer.prototype.hideHighlightedMargin = function(annotations) {
      var marginObjects;
      this.highlightedObjects = [];
      marginObjects = annotations ? jQuery.map(annotations, function(val, i) {
        return val._marginObject;
      }) : [];
...
//
//# sourceMappingURL=annotator.marginviewer.map