...
def run():
	...
	lulu,created = User.objects.get_or_create(username=u'lulu',password=make_password('x'),email=u'lulu@lili.org',first_name=u'Lucie',last_name=u'Rivallant-Delabie')
	romain,created = User.objects.get_or_create(username=u'linux2400',password=make_password('x'),email=u'lulu@lili.org',first_name=u'Romain',last_name=u'Nguyen van yen')
	damien,created = models.Human.objects.get_or_create(name=u'Damien Poinsard', description=u'Theaterpedagog', geo=berlin, parent=enscene)
	marjo,created = models.Human.objects.get_or_create(name=u'Marjorie Nadal', description=u'Theaterpedagogin', geo=berlin, parent=impro)
	...
	# create some events
	x1,created = models.Meeting.objects.get_or_create(parent=enscene,name=u'En Scène 2012-2013, Ubu Roi',description=u'Atelier en Scène 2012-2013. Mise en scène de la pièce UBU ROI d\'Alfred Jarry',geo=thbm,start=datetime(2012, 10, 11, 20, 00, 00),end=datetime(2012, 10, 11, 22, 00, 00),rule=weekly)
	if(created):
		x1.participants.add(lulu)
		x1.participants.add(romain)
	x1,created = models.Meeting.objects.get_or_create(parent=fnc,name=u'Distribution',description=u'',geo=stpierre,start=datetime(2014, 12, 14, 17, 30, 00),end=datetime(2014, 12, 14, 20, 00, 00),rule=weekly)
	if(created):
		x1.participants.add(romain)
	x1,created = models.Meeting.objects.get_or_create(parent=fnc,name=u'Cuisine',description=u'',geo=lejeune,start=datetime(2014, 12, 14, 13, 00, 00),end=datetime(2014, 12, 14, 17, 30, 00),rule=weekly)
	if(created):
		x1.participants.add(romain)
	# add some wikis
	Wiki.objects.get_or_create(name=u'Wikipédia francophone',url='http://fr.wikipedia.org/w/',slug='wikipedia.fr',language=fr)
	Wiki.objects.get_or_create(name=u'English Wikipedia',url='http://en.wikipedia.org/w/',slug='wikipedia.com',language=en)
	Wiki.objects.get_or_create(name=u'English Wikihow',url='http://www.wikihow.com/',slug='wikihow.com',language=en)
	Wiki.objects.get_or_create(name=u'Wikihow francophone',url='http://fr.wikihow.com/',slug='wikihow.fr',language=fr)
	Wiki.objects.get_or_create(name=u'Wikiversity',url='http://en.wikiversity.org/w/',slug='wikiversity.com',language=en)
	Wiki.objects.get_or_create(name=u'Wikiversité',url='http://fr.wikiversity.org/w/',slug='wikiversity.fr',language=fr)

	# install some wiki sources
	make_from_slug(models.WikimediaArticle,enscene,'wikipedia.com/Ubu_Roi')
